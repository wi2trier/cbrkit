...
synthesizer = cbrkit.synthesis.build(
    provider,  # type: ignore[arg-type]  # ty: ignore[invalid-argument-type]
    prompt,
)
...
pooling_func = cbrkit.synthesis.pooling(provider, pooling_prompt)  # type: ignore[arg-type]  # ty: ignore[invalid-argument-type]
get_result = cbrkit.synthesis.chunks(synthesizer, pooling_func, size=10)  # ty: ignore[invalid-argument-type]
