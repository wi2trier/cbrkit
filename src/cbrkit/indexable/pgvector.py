...
base with two system columns that are populated automatically from a
user-designated text column:

- ``pgvector_column`` (:class:`PGVECTOR`) — dense embedding of the text
  column, used for ANN search.
- ``tsvector_column`` (:class:`TSVECTOR`, generated by Postgres from the text
  column) — sparse FTS index target.
...
Declarative-ORM hosts that manage their own table DDL can compose
:class:`PGVECTOR` and :class:`TSVECTOR` (re-exported from this module for
convenience) directly in their model and pass ``MyClass.__table__`` to
:class:`pgvector_async`::
    ...
    class Chunk(Base):
        ...
        body: Mapped[str]
        body_vec: Mapped[Any] = mapped_column(PGVECTOR(384), nullable=False)
...
class pgvector_async[K: int | str, V = Mapping[str, Any]](sqlalchemy_async[K, V]):
    ...
    the user's *text_column*:

    - ``pgvector_column`` carries a dense embedding (computed in Python via
      :paramref:`conversion_func` and inserted on write).
    - ``tsvector_column`` is a Postgres-generated ``TSVECTOR`` over the text
      column.

    Args:
        text_column: User column whose text is embedded and FTS-indexed.
            Required for any non-empty *index_type*.
    ...
    def _validate_init(self) -> None:
        ...
        if self.text_column is None:
            raise ValueError("text_column is required for pgvector_async")
        if self.table is None and self.text_column not in self.columns:
            raise ValueError(
                f"text_column={self.text_column!r} must be declared in `columns` "
                "when building the schema from a column spec"
            )
        ...
        if self.has_sparse:
            ...
            cols.append(
                sa.Column(
                    ...
                    TSVECTOR(),
                    _tsvector_computed_clause(self.text_column, self.tsvector_config),
                    nullable=False,
                )
            )
        return cols
    ...
    async def reembed_all(self, batch_size: int = 1000) -> int:
        ...
        tc = self._table.c[self.text_column]
        total = 0
